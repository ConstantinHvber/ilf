...
if False:
    def process_files_first_attempt(
        buckets: Dict[str, List[Path]], sources_dir: Path, output_dir: Path
    ):
        for version, sources in tqdm(buckets.items()):
            versioned_dir = output_dir / version.replace(".", "_")
            contracts_dir = versioned_dir / "contracts"
            migrations_dir = versioned_dir / "migrations"
            build_dir = versioned_dir / "build/contracts"
            contracts_dir.mkdir(parents=True, exist_ok=True)
            migrations_dir.mkdir(parents=True, exist_ok=True)
            build_dir.mkdir(parents=True, exist_ok=True)

            # for verb in ["install", "use"]:
            #     assert 0 == subprocess.call(f"solc-select {verb} {version}".split()), f"Version {version} failed"

            # subprocess.call(f"solc ".split())

            mains = []

            for source_file in tqdm(sources, leave=False):
                try:
                    contracts, main_contract = make_contracts_unique(
                        sources_dir / source_file, contracts_dir / source_file
                    )
                    mains.append(main_contract)
                except IndexError as e:
                    print("Error", source_file)
                    continue

            truffle_config = versioned_dir / "truffle-config.js"
            truffle_config.open("w").write(
                TRUFFLE_CONFIG_CONTENTS_FMT.format(version=version)
            )
            ...
            deploy_file = migrations_dir / "2_deploy_contracts.js"
            deploy_file.open("w").write(truffle_deploy_generate(contracts=mains))
    ...
    solidity_keyword_re = re.compile("\\b(" + "|".join(solidity_keywords) + ")\\b")
    "(address|string\\d*|bytes\\d*|int\\d*|uint\\d*|bool|hash\\d*)"
    SOLIDITY_BLOCK_COMMENT_FMT = """
    /*
    {}
    */
    """
    ...
    def make_contracts_unique(source_file: Path, output_file: Path, salt=None) -> List[str]:
        "Returns all replaced symbols and the one we consider as 'main' - quite hacky"
        salt = salt or source_file.stem

        with open(source_file) as fr, open(output_file, "w") as fw:
            contents = fr.read()

            names_unfiltered = contract_name_re.findall(contents)
            names = list(filter(lambda n: n not in solidity_keywords, names_unfiltered))
            if len(names) == 0:
                import ipdb

                ipdb.set_trace()

            main_contract, main_replacement = names[-1], None

            # try to avoid inclusions by going from longest to shortest word
            names.sort(key=len, reverse=True)
            replacements = {}

            # and hash everything like crazy
            letter_prefix = chr(
                0x41
                + (
                    (
                        int(source_file.stem, 16)
                        if can_be_hex_number(source_file.stem)
                        else int.from_bytes(bytes(source_file.name, "utf8"))
                    )
                    % 26
                )
            )
            for name in names:
                digest = hashlib.sha256((name + salt).encode("utf8")).hexdigest()
                replacement = f"{letter_prefix}_{digest}"

                if name == main_contract:
                    replacement += "_MAIN"  # just in case for manual debugging
                    main_replacement = replacement

                contents = contents.replace(name, replacement)
                replacements[replacement] = name

            # caveat 1: what if the number of found occurrences rises during replacement?
            # ex. names = [SensibleContractName, beef], and 'beef' could be inside some hash of 'SensibleContractName'
            # caveat 2: we might be changing some variable names

            contents += SOLIDITY_BLOCK_COMMENT_FMT.format(pformat(replacements))

            fw.write(contents)

        return list(replacements.keys()), main_replacement


    def can_be_hex_number(repr: str) -> bool:
        try:
            int(repr, 16)
            return True
        except ValueError:
